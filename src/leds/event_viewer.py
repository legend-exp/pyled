...
class event_viewer:
    ...
    def get_phy_event_position(self, period, run, file_tstamp, idx):
        ...
        for file in files:
            if file in list(self.events_per_file_cache):
                ...
                n_events = sto.read_n_rows(
                    f"ch{baseline_channel}/raw/baseline",
                    file.replace(
                        self.config["tier_hit"], self.config["tier_raw"]
                    ).replace("hit", "raw"),
                )
        ...
        start_r = idx - cum_events[file_n - 1] if file_n > 0 else idx
        file = files[file_n]
        return (
            file.replace(self.config["tier_hit"], self.config["tier_raw"]).replace(
                "hit", "raw"
            ),
            file.replace(self.config["tier_hit"], self.config["tier_dsp"]).replace(
                "hit", "dsp"
            ),
            file,
            start_r,
    ...
    def _get_event(self, period, run, file_tstamp, idx):
        ...
        self.working_dets = [
            ...
            if self.chmap[det]["analysis"]["processable"] is True
        ]
        self.energy_dict = {
            det: sto.read_object(
                f"ch{self.chmap[det].daq.rawid}/hit/cuspEmax_ctc_cal",
                self.hit_file,
                start_row=self.index,
                n_rows=1,
            )[0].nda[0]
            if det in self.working_dets
            else None
            for det in self.dets
        }
    ...
    def plot_event(self, period, run, file_tstamp, idx):
        ...
        if strings_dict == self.strings_dict:
            ...
            xs, ys = get_plot_source_and_xlabels(
                chan_dict, channel_map, self.strings_dict, ΔR=160, ΔH=40
            )
            patches = []
    ...
    def plt_detector_plot(self, patches, ctitle="", plot_title="LEGEND event view"):
        minvalue = 25
        maxvalue = 6000
