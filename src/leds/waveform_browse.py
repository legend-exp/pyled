...
def plot_string_compressed(fig, ax, ev, browsers, line_parameter="wf_blsub", string=1):
    ...
    dets = [det_dict["name"] for pos, det_dict in ev.chmap.map("location.string", unique=False)[string].map("location.position").items() if det_dict["analysis"]["processable"]==True]
...
def plot_above_threshold_compressed(fig, ax, ev, browsers, line_parameter="wf_blsub", string=1):
    def onpick(event, lined):
        legline = event.artist
        if event.mouseevent.dblclick:
            for entry, line in lined.items():
    ...
    dets = [det for det, energy in ev.energy_dict.items() if energy is not None and (energy>25)]
...
def plot_all_compressed(fig, ax, ev, browsers, line_parameter="wf_blsub"):
        ...
        if event.mouseevent.dblclick:
            for entry, lines in lined.items():
                if entry is not legline:
                    ...
                    entry.set_alpha(1)
                    for line in lines: line.set_visible(True)
        else:
            lines = lined[legline]
            # If the label of the legend matches that of the line, then set the alpha to 0
            # Change the alpha on the line in the legend, so we can see what lines
            ...
            legline.set_alpha(1.0 if visible else 0.2)
            for line in lines: line.set_visible(visible)
        fig.canvas.draw()
    strings = {}
    lines=[]
    ...
    for string in np.unique(list(strings)):
        plt.plot([], color = mpl.colormaps["tab20"](string/12), label= f"String:{string:02}", lw=5)
...
def plot_event_compressed(ev, fig, browsers, line_parameter="wf_blsub", plot_category="all"):
    ax = fig.add_subplot(111)
    for det, browser in browsers.items():
    ...
    if plot_category == "all":
        ...
        plot_string_compressed(fig, ax, ev, browsers, line_parameter, string=int(plot_category[-2:]))
...
def plot_string_exploded(fig, ev, browsers, line_parameter="wf_blsub", string=1):
    ...
    if len(dets)<6:
        ...
        for i, det in enumerate(dets[1:]):
            det_ax[det]=plt.subplot(math.ceil(len(dets)/2),2, i+2,
        ...
        for i, det in enumerate(dets[1:]):
            det_ax[det]=plt.subplot(math.ceil(len(dets)/3),3, i+2,
                                   sharex=det_ax[dets[0]],
                                    sharey=det_ax[dets[0]],
                                   )
...
def plot_above_threshold_exploded(fig, ev, browsers, line_parameter="wf_blsub", string=1):
    dets = [det for det, energy in ev.energy_dict.items() if energy is not None and (energy>25)]
    if len(dets)>12:
        dets = dets[:12]
    if len(dets) ==1:
...
def plot_all_exploded(fig, ev, browsers, line_parameter="wf_blsub"):
    ...
    for i,string in enumerate(sorted(string_list)[1:]):
        strings[string]= plt.subplot(4,3,i+2,sharex=strings[sorted(string_list)[0]],
    ...
    for string in list(strings):
        ax = strings[string]
        ax.scatter([], [],color = "white", label= f"String:{string:02}", marker="x", s=0.0001)
        ax.legend(ncol=1, loc = "upper left", 
                    frameon=False, markerfirst=False)#
...
def plot_event_exploded(ev, fig, browsers, line_parameter="wf_blsub", plot_category="all"):
    for det, browser in browsers.items():
        browser.find_entry(ev.index, append=False)
    if plot_category == "all":
        ...
        plot_string_exploded(fig, ev, browsers, line_parameter, string=int(plot_category[-2:]))
...
class spoof_iterator():
    ...
    def read(self, entry):
        self.lh5_buffer, n_rows = self.sto.read_object(f'{self.channel}/raw', 
                self.ev.raw_file, start_row = self.ev.index, n_rows=1)
...
class event_waveform_browser():
    ...
    def _get_browser(self, channel, dsp_config):
        browser =  WaveformBrowser(self.ev.raw_file, 
                                    f"{channel}/raw", 
    ...
    def get_browsers(self):
        ...
        dataprod_config = self.ev.meta.dataprod.config.on(os.path.basename(self.file).split("-")[4])
        dsp_configs = dataprod_config["snakemake_rules"]["tier_dsp"]["inputs"]["processing_chain"]
        ...
        if channels == list(self.browsers):
                ...
                if dsp_configs[f"{channel}/raw"] != self.current_dsp_configs[channel]:
                    if channel in list(self.cached_browsers):
                        ...
                        if dsp_configs[channel] in list(self.cached_browsers[channel]):
                            self.browsers[channel] = self.cached_browsers[channel].pop(dsp_configs[f"{channel}/raw"])
                        else:
                            self.browsers[channel] = self._get_browser(channel, dsp_configs[f"{channel}/raw"])
                    else:
                        self.cached_browsers[channel] = {self.current_dsp_configs[channel]:self.browsers.pop(channel)}
                        self.browsers[channel] = self._get_browser(channel, dsp_configs[f"{channel}/raw"])
            ...
            for channel in channels:
                if channel not in list(self.browsers):
                    if channel in list(self.cached_browsers):
                        if dsp_configs[f"{channel}/raw"] in list(self.cached_browsers[channel]):
                            self.browsers[channel] = self.cached_browsers[channel][dsp_configs[f"{channel}/raw"]].pop()
                        else:
                            self.browsers[channel] = self._get_browser(channel, dsp_configs[f"{channel}/raw"])
                    else:
                        self.browsers[channel] = self._get_browser(channel, dsp_configs[f"{channel}/raw"])
            # check for extra channels
            for channel, browser in self.browsers.items():
                if channel not in channels:
                    if channel in list(self.cached_browsers):
                        self.cached_browsers[channel].update({self.current_dsp_configs[channel]:self.browsers.pop(channel)})
                    else:
                        self.cached_browsers[channel] = {self.current_dsp_configs[channel]:self.browsers.pop(channel)}
        self.current_dsp_configs = {name.split("/")[0]:item for name, item in dsp_configs.items() }
